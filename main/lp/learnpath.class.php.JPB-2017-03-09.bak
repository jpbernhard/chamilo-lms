    ...
    public function get_toc()
    {
        ...
        foreach ($this->ordered_items as $item_id) {
            ...
            $toc[] = array (
                ...
                'status'        => $this->items[$item_id]->get_status(true, true),
    ...
    public function set_previous_step_as_prerequisite_for_all_items()
    {
       ...
       // $item = array();
        ...
        if (!empty($this->items)) {
            ...
            /* Old Function... Not in order...
            // Francois Belisle Kezber


            foreach ($this->ordered_items as $item) {
            ...
            */
            ...
            /* New function in order...
            // Francois Belisle Kezber
            // Cannot assign previous item as prerequesite if
            // the items are not sorted first!!!
            // It is the exact same code, but instead of going thru each $items in the order they were entered,
            // We go thru them in order of display thru the $ordered_item indexes....
            */

            $this->ordered_items = self::get_flat_ordered_items_list($lp_id, 0, $course_id);
            $index = 0;
            while ( !empty($this->ordered_items[$index])) {
              $item = $this->items[$this->ordered_items[$index]];
