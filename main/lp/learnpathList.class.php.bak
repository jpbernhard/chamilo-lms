...
// FB Kezber
// Originally used the iid, but it caused problemes when returning the list of learning paths
// as the iids are not used as FK anywhere... I could have either changed the function returning the lp list
// or simply applied this fix. Either way, there should not be 2 lp_id with the same number for the same course.
// IF THERE'S TROUBLE, SIMPLY REVERT TO THE IID as the index for the list, and change the function that returns
// the flat list to make sure it returns the id and not the iid.
//            $this->list[$row->getIid()] = array(


            $this->list[$row->getid()] = array(
