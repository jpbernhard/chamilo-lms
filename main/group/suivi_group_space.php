...
echo '&nbsp;Le '.$now.' <a href="#" onclick="window.print()"><img align="absbottom" src="../img/printmgr.gif">&nbsp;'.get_lang('Print').'</a>';
echo  '&nbsp; <a target="_blank" href="save_diff_all.php"><img src="'.api_get_path(WEB_IMG_PATH).'addd.gif" border="0" />'.get_lang('save_diff_all').'</a>' ;
echo '</div>';
...
$current_group_result = api_sql_query($sql,__FILE__,__LINE__);
...
unset($all_datas)    ;   
 ...
 //on cherche les examens
    ...
    $sqlexam = "SELECT  mod_no
          ...
          api_sql_query("INSERT INTO $tbl_stats_exercices_temp
...
//pours chaque cours dans lequel il est inscrit, on cherche les jours complétés
                ...
                FROM  c_lp_item_view
                       ...
                       AND lp_view_id =  '$lp_id_view'
                        AND c_id = '$c_id_view'
                         ...
                         ";
                  $res5 = api_sql_query($Req5);// or die(mysql_error());
                          ...
                          $max = $resul5['0'];
                                ...
                                $Req6= "SELECT COUNT( id )
                                 ...
                                 WHERE  id = '$max'
                                  AND c_id = '$c_id_view'
                                     ";
                                $res6 = api_sql_query($Req6);// or die(mysql_error());
 ...
 // fin affichage des jours complétés dans les parcours par chaque élève
  ...
  $sqlexamtot = "SELECT  mod_passed
                                 ...
                                 WHERE exe_user_id = '$user_in_groupe'
                                                                 ";
                    $resultexamtot = api_sql_query($sqlexamtot);
...
//on affiche la différence  
 ...
 if ($diff >0) $sing= '<b><font color=#CC0000>';        
  else $sing = '<b><font color=#008000>' ;                 
 if ($diff >0) $avertissement= get_lang('retard')  ; 
   else $avertissement =get_lang('avance') ; 
 if ($diff >0) $ajout= '-'  ; 
 else $ajout ='' ;
