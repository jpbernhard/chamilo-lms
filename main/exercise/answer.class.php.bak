    ...
    public function duplicate($newQuestionId, $course_info = null)
    {
        ...
        if ($this->nbrAnswers) {
            ...
            for ($i=1;$i <= $this->nbrAnswers;$i++) {
                ...
                $newid[$i] = $quizAnswer->getIid();

                $em->flush();
            }

            for ($i=1;$i <= $this->nbrAnswers;$i++) {
               for ($x=1;$x <= $this->nbrAnswers;$x++) {
                 if ($oldids[$x] == $oldanswers[$i])
                 {
                   $sql = 'Update c_quiz_answer set correct = ' . $newid[$x] . ' where iid = ' . $newid[$i] . " and correct = '" . $oldanswers[$i] . "'";
                   //echo '<h1>' . $sql . '</h1>';
                   Database::query($sql);
                 }
               }
